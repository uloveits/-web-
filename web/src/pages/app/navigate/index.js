...
export default class AppNavigatePage extends React.Component {
  static propTypes = {
    location: P.any,
    history: P.any,
    actions: P.any,
  ...
  getTblList = async () => {
    ...
    let res = await settingApi.navigateList(params);
    console.log('所有首页导航信息');
    console.log(res)
    this.setState({
      list:res.data,
      loading:false,
  ...
  delete = async(data) => {
    ...
    let res = await settingApi.deleteNavigate({id:data.id})
    if(res.code === '200') {
  ...
  onOK = async () => {
    ...
    if(modalTitle === '新增'){
      ...
      let res = await settingApi.addNavigate(param);
      if(res.code === '200') {
      ...
      let res = await settingApi.updateNavigate(param);
      if(res.code === '200') {
      ...
      <div className='page-content'>
        ...
        <div className='pd10'>
          <div className='flex-left'>
            <Button type='primary' onClick={this.add.bind(this)}>新增导航</Button>
          </div>
          <KtSeparator />
          <Table
        ...
        >
          <div>
            ...
            <KtSeparator />
            <Row type="flex" justify="center" align="middle">
              <Col span={5} className='text-right pr10'>路由：</Col>
              <Col span={10}>
                <Input placeholder="请输入路由链接" onChange={onInputChange.bind(this,'param','route')} value={this.state.param.route}/>
              </Col>
            </Row>
            <KtSeparator />
            <Row type="flex" justify="center" align="middle">
              <Col span={5} className='text-right pr10'>上线/下线：</Col>
              <Col span={10}>
